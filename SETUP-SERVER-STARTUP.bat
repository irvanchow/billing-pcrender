...
powershell -Command "$ws = New-Object -ComObject WScript.Shell; $s = $ws.CreateShortcut('%STARTUP_DIR%\IDB-RentalServer.lnk'); $s.TargetPath = 'pythonw'; $s.Arguments = '-m server.main'; $s.WorkingDirectory = '%PROJECT_DIR%'; $s.WindowStyle = 7; $s.Save()"
